...
# ============================================================================
# DEBATE SCHEMAS - Multi-Agent Debate System
# ============================================================================

class AgentConfig(BaseModel):
    """Configuration for a debate agent"""
    name: str = Field(..., description="Agent name (e.g., 'Planner')")
    persona_file: str = Field(..., description="Filename in prompts/agent_personas/")
    emoji: str = Field(..., description="Emoji for UI display")
    role_description: str = Field(..., description="One-line description of agent's role")


class DebateMessage(BaseModel):
    """Single message in debate history"""
    round: int = Field(..., ge=1, le=3, description="Round number (1=proposal, 2=rebuttal, 3=vote)")
    agent: str = Field(..., description="Agent name")
    message_type: Literal["proposal", "rebuttal", "vote"] = Field(..., description="Type of message")
    content: str = Field(..., description="Agent's statement or vote")
    timestamp: Optional[str] = Field(default=None, description="ISO timestamp")


class DebateState(BaseModel):
    """State that flows through debate nodes"""
    ...
    proposals: Dict[str, str] = Field(default_factory=dict, description="agent_name → proposal_text")
    rebuttals: Dict[str, str] = Field(default_factory=dict, description="agent_name → rebuttal_text")
    votes: Dict[str, str] = Field(default_factory=dict, description="agent_name → voted_for")
    ...
    # Params (extracted from winner)
    params: Optional[Dict[str, Any]] = None
    
    # Metadata
    agent_initiated: bool = Field(default=False, description="True if workflow was proactively generated")
...
class OrchestratorState(BaseModel):
    ...
    agent_initiated: bool = Field(
        default=False,
        description="True if this workflow was proactively generated by monitoring"
    )
    ...
    # Debate output (NEW - replaces extractor)
    debate_state: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Nested debate state with proposals, rebuttals, votes"
    )
    winning_agent: Optional[str] = Field(default=None, description="Agent whose strategy won")
    winning_strategy: Optional[str] = Field(default=None, description="Winning proposal text")
    vote_tally: Optional[Dict[str, int]] = Field(default=None, description="Vote counts per agent")
    debate_history: List[Dict[str, Any]] = Field(default_factory=list, description="Full debate transcript")
    
    # Extractor/Executor output
