...
export default function ApprovalsPage() {
      ...
      const parsed = data.approvals.map((item: any) => {
        ...
        try {
          if (item.preview_data) {
            previewData =
              typeof item.preview_data === 'string'
  ...
  const handleApprove = async (id: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/approval/${id}/decide`,
        {
          method: 'POST',
  ...
  const getApprovalTitle = (item: ApprovalGate) => {
    if (item.template === 'matching') {
      ...
      const count =
        preview.proposed_assignments || preview.assignments?.length || 0
  ...
  const getApprovalSummary = (item: ApprovalGate) => {
    if (item.template === 'matching') {
      ...
      const count = preview.total_analyzed || preview.totalAnalyzed || 0
      const metricCount =
        preview.metric_count || Object.keys(preview.metrics || {}).length
  ...
  const renderTemplatePreview = (item: ApprovalGate) => {
    if (item.template === 'matching') {
      const preview = item.preview as any
      const assignments =
        preview.assignments_preview || preview.assignments || []
      return (
        <div>
        ...
        <div>
          ...
          <div className="space-y-2 overflow-y-auto max-h-60">
            {flaggedItems.slice(0, 10).map((flagged: any, i: number) => (
              <div
                key={i}
                className="p-3 border rounded-lg bg-white/5 border-white/10"
              >
    ...
    if (item.template === 'analysis') {
      ...
      const reportSummary =
        preview.report_summary || `Analysis of ${recordsCount} records`
              ...
              >
                ...
                !Array.isArray(metricValue) ? (
                  ...
                  <div className="space-y-1">
                    {Object.entries(metricValue as Record<string, any>)
                      .slice(0, 5)
                      .map(([group, value]) => (
                        <div
                          key={group}
                    ...
                    {Object.keys(metricValue as Record<string, any>).length >
                      5 && (
                      <p className="text-xs text-white/50">
                        +
                        {Object.keys(metricValue as Record<string, any>)
                          .length - 5}{' '}
                        more
                      </p>
                    )}
      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-white/70">
            Review and confirm Foundry&rsquo;s suggested actions before they go
            live.
          </p>
          {pendingItems.length > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 mt-4 border rounded-lg bg-yellow-400/10 border-yellow-400/30 w-fit">
              ...
              <p className="font-medium text-yellow-300">
                {pendingItems.length}{' '}
                {pendingItems.length === 1 ? 'action' : 'actions'} require your
                review
              </p>
            </div>
          )}
                ...
                <div className="space-y-4">
                        ...
                        <Badge
                          variant={
                            item.template === 'matching'
                              ? 'info'
                              : item.template === 'monitoring'
                                ? 'default'
                                : 'success'
                          }
                        >
                          {item.template.charAt(0).toUpperCase() +
                            item.template.slice(1)}
                        </Badge>
                  ...
                  {activeTab === 'approved' && item.workflowRunId && (
                    <div className="flex flex-wrap gap-3 pt-2">
                      <Button
                        onClick={() =>
                          router.push(`/results/${item.workflowRunId}`)
                        }
                        size="md"
                      >
                        <CheckCircle2 className="inline-block w-4 h-4 mr-2" />
